...
for smell_word in SMELL_WORDS:
    ...
    print(smell_word)
    output_context.write("Contexts for "+smell_word+"\n")
    for cxword, count in list(space[smell_word].items()):
        print(cxword, ":", count)
        output_context.write(cxword+":"+str(count)+"\n")
    smell_words_cosine_all = [(t,cosine(space,smell_word,t)) for t in set(tokens) if cosine(space, smell_word, t) != "" and cosine(space, smell_word, t) > 0]
    smell_words_cosine = sorted(smell_words_cosine_all,key=lambda x:(-x[1],x[0]))
    smell_words_cosine = [(w,c) for (w,c) in smell_words_cosine if c > 0.5 and w != smell_word]
