...
file_out1 = "distances_smellwords_impliedwords_context100_Acton20.csv"
file_out2 = "distances_smellwords_allwords_context100_Acton20.csv"
...
files = [f for f in listdir(dir_in) if isfile(join(dir_in, f)) and f.endswith('.txt') and f.startswith('Acton')] #Deborah: change
files = files[0:20] #Deborah: change
...
context_size = 100 #Deborah: change
...
output1 = csv.writer(open(os.path.join(dir_out, file_out1), 'w'), delimiter=',', quoting=csv.QUOTE_MINIMAL)
output1.writerow(['target_word', 'synonym',  'cosine_distance'])
...
for implied in IMPLIED_SMELL_WORDS:
    ...
    for smell_word in SMELL_WORDS:
        ...
        output1.writerow([implied, smell_word, cosine_implied_smell])
...
output2 = csv.writer(open(os.path.join(dir_out, file_out2), 'w'), delimiter=',', quoting=csv.QUOTE_MINIMAL)
output2.writerow(['target_word', 'synonym',  'cosine_distance'])
...
for smell_word in SMELL_WORDS:
    ...
    for i in range(len(smell_words_cosine)):
        if get_new_pos(smell_words_pos[i][0][1]) in ["n"]:  # "["ADJ", "N", "V"]:
            ...
            output2.writerow([smell_word, smell_words_cosine[i][0], smell_words_cosine[i][1]])
